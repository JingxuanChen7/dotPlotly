...
# opt <- list(
#  input_filename = "~/Downloads/tmp/mummer/mum_flye_S288c_czaja_227.filter.coords",
#  output_filename = "~/Downloads/tmp/mummer/out",
#  min_query_aln = 500,
#  min_align = 500,
#  plot_size = 15,
#  h_lines = FALSE,
...
# strain name
strain_name <- basename(opt$input_filename)
strain_name <- sub("\\.(.*)$","",strain_name,perl = T)
...
if (opt$similarity) {
  gp = ggplot(alignments) +
    geom_rect(aes(xmin=521151, xmax=521221, ymin=0, ymax=Inf, fill="0")) +
    geom_rect(aes(xmin=521238, xmax=521313, ymin=0, ymax=Inf, fill="1")) +
    geom_rect(aes(xmin=521311, xmax=521393, ymin=0, ymax=Inf, fill="2")) +
    geom_rect(aes(xmin=521394, xmax=521726, ymin=0, ymax=Inf, fill="3")) +
    geom_rect(aes(xmin=521727, xmax=521785, ymin=0, ymax=Inf, fill="2")) +
    geom_rect(aes(xmin=521813, xmax=522999, ymin=0, ymax=Inf, fill="4")) +
    geom_rect(aes(xmin=523019, xmax=523342, ymin=0, ymax=Inf, fill="5")) +
    geom_rect(aes(xmin=523429, xmax=523510, ymin=0, ymax=Inf, fill="6")) +
    scale_fill_manual(values = c("0"="#999999","1"="#fff2ae","2"="#fddbc7","3"="#e6f598","4"="#ef8a62","5"="#e0f3f8","6"="#d9d9d9"), labels = c("0"="tRNA-Gly", "1"="s22+_TY3","2"="s23-_TY1","3"="s24+_TY2","4"="DRT2","5"="s26-_TY1","6"="tRNA-Ser"), name ="Legend" ) +
    geom_point(
    ...
    #      )) +
    xlab("UWOPS05-227.2_Chr4_DRT2") +
    ylab(paste0(strain_name,"_local_contig")) 
} else {
  gp = ggplot(alignments) +
    geom_rect(aes(xmin=521151, xmax=521221, ymin=0, ymax=Inf, fill="0")) +
    geom_rect(aes(xmin=521238, xmax=521313, ymin=0, ymax=Inf, fill="1")) +
    ...
    xlab("UWOPS05-227.2_Chr4_DRT2") +
    ylab(paste0(strain_name,"_local_contig")) 
  #geom_rect(aes(xmin=520000, xmax=524000, ymin=0, ymax=Inf), alpha = 0.05) +
}
# gp
